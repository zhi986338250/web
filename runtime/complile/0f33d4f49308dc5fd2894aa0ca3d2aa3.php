...
<section class="pa1">
  <div class="tm3"><img src="{content:ico}" alt="{content:title}" /></div>
  <div class="tm1">{content:title}</div>
  <div class="tm4" id="maximg"> {content:content} </div>
  <div class="f_l">
    <ul>
      <li> {pboot:if('{content:prelink}'=='javascript:;')}
        上一页：暂无
        {else} <a href="{content:prelink}">上一页</a> {/pboot:if} </li>
      <li> {pboot:if('{content:nextlink}'=='javascript:;')}
        下一页：暂无
        {else} <a href="{content:nextlink}">下一页</a> {/pboot:if} </li>
    </ul>
  </div>
</section>
